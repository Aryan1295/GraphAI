...
flags.DEFINE_integer('dim_1', 18,
                     'Size of output dim (final is 2x this, if using concat)')
flags.DEFINE_integer('dim_2', 9,
                     'Size of output dim (final is 2x this, if using concat)')
...
flags.DEFINE_string('base_log_dir', '../log_dir',
                    'base directory for logging and saving embeddings')
flags.DEFINE_integer('validate_iter', 10,
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    ...
    while not finished:
        feed_dict_val, finished, edges = minibatch_iter.incremental_embed_feed_dict(
            size, iter_num)
        iter_num += 1
        outs_val = sess.run([model.loss, model.mrr, model.outputs2],
                            feed_dict=feed_dict_val)
    ...
    np.save(out_dir + name + mod + "_try.npy", val_embeddings)
    with open(out_dir + name + mod + "_try.txt", "w") as fp:
        fp.write("\n".join(map(str, nodes)))
...
def construct_placeholders():
    ...
    placeholders = {
        ...
        'neg_samples':
        tf.placeholder(tf.int32, shape=(None, ), name='neg_sample_size'),
        'dropout':
...
save_path1 = os.path.join(save_dir, 'best_validation')
...
saver.save(sess, save_path=save_path1, global_step=total_steps)
