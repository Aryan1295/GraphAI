...
def load_data(prefix, normalize=True, load_walks=False):
    ...
    if os.path.exists(prefix + "-feats.npy"):
        ...
        feats = None
    tem1 = json.load(open(prefix + "-id_map.json"))
    tem2 = ast.literal_eval(tem1)
    id_map = {conversion(k): int(v) for k, v in tem2.items()}
    walks = []
    temp = json.load(open(prefix + "-class_map.json"))
    class_map = ast.literal_eval(temp)
